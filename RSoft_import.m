...
%% uniterleave output fields
        ...
        filename = ['1x7PLx27SMF-asy-Lopt-Degen-Rev1-0', num2str(mm-1), '.fld'];
        ...
        for j = 1:sz_f_px
            for k = 1:sz_f_px
                OutputFields(k,j,mm) = M(j,2*k-1) + 1i*M(j,2*k);    % k & j are swapped because in MATLAB first index is row index, i.e. y-coordinate
            end
        end
  
        OutputFields(:,:,mm) = flipud(OutputFields(:,:,mm));     % flip upside-down is necessary because in RSoft y-coordinate increases from bottom to top
