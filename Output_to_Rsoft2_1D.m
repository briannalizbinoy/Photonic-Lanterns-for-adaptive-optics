function Output_to_Rsoft2_1D(field,filename,part,size,lambda) %field=imaginary field distribution, filename, size must be the same in x and y, 
% field = A_temp;
...
%filename = fullfile(savepath, 'myFile.txt');
    ...
    %441 -44 44
    part3=[num2str(length(field(:))) ' ' '-' num2str(size/2*1E6) ' ' num2str(size/2*1E6) ' 0 OUTPUT_' part ' Wavelength=' num2str(lambda*1E6)];
    %part4=[num2str(length(field(1,:))) ' ' '-' num2str(size/2*1E6) ' ' num2str(size/2*1E6)];
            
    fprintf(fid, '%s\n%s\n', part1,part3);  % header
    for i=1:length(field(:))
        %for j=1:length(field(:,1))
            fprintf(fid,'%E\t%E\t',real(field(i)),imag(field(i)));
